...
class MetalTexture: NSObject {
  ...
  func loadTexture(device device: MTLDevice, commandQ: MTLCommandQueue, flip: Bool){
    ...
    let context = CGBitmapContextCreate(nil, width, height, bitsPerComponent, rowBytes, colorSpace, CGBitmapInfo(rawValue: CGImageAlphaInfo.PremultipliedLast.rawValue).rawValue)
  ...
  func image(mipLevel mipLevel: Int) -> UIImage{
    ...
    let context = CGBitmapContextCreate(p, mipmapedWidth, mipmapedHeight, 8, rowBytes, colorSpace, CGBitmapInfo(rawValue: CGImageAlphaInfo.PremultipliedLast.rawValue).rawValue)
